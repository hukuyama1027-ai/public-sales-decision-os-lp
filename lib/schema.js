...
const EVENT_COLUMNS = Object.freeze({ client_key:'TEXT', entity_type:'TEXT', entity_id:'TEXT', metadata_json:'TEXT' });
const CR004_COLUMNS = Object.freeze({
  events: Object.freeze({ release_version:'TEXT' }),
  watch_items: Object.freeze({ source_hash_snapshot:'TEXT' })
});

async function run(db, sql) { return db.prepare(sql).run(); }
async function ensureBase(db) { for (const sql of BASE_DDL) await run(db, sql); }
...
async function ensureColumns(db, table, columns) {
  ...
  for (const [name,type] of Object.entries(columns)) {
    ...
    try { await run(db, `ALTER TABLE ${table} ADD COLUMN ${name} ${type}`); }
    catch (err) {
...
export async function ensureCr003Schema(db) {
  ...
  await ensureBase(db);
  await ensureColumns(db, 'events', EVENT_COLUMNS);
  for (const sql of CR003_DDL) await run(db, sql);
  const existing = await db.prepare("SELECT version FROM schema_migrations WHERE version='cr003' LIMIT 1").first();
  if (!existing) await db.prepare('INSERT OR IGNORE INTO schema_migrations(version,applied_at) VALUES(?,?)').bind('cr003',new Date().toISOString()).run();
  return { applied: !existing, version:'cr003' };
}

export async function ensureCr004Schema(db) {
  if (!db) throw new Error('DB_UNAVAILABLE');
  await ensureCr003Schema(db);
  const existing = await db.prepare("SELECT version FROM schema_migrations WHERE version='cr004' LIMIT 1").first();
  await ensureColumns(db, 'events', CR004_COLUMNS.events);
  await ensureColumns(db, 'watch_items', CR004_COLUMNS.watch_items);
  await run(db, 'CREATE INDEX IF NOT EXISTS idx_events_release_created ON events(release_version, created_at)');
  if (!existing) await db.prepare('INSERT OR IGNORE INTO schema_migrations(version,applied_at) VALUES(?,?)').bind('cr004',new Date().toISOString()).run();
  return { applied: !existing, version:'cr004' };
}

export const __test = { BASE_DDL, CR003_DDL, EVENT_COLUMNS, CR004_COLUMNS, ensureColumns };
