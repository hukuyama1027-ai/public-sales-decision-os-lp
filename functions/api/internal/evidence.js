...
export async function onRequestGet({request,env}){const token=bearerToken(request);if(!token)return json({ok:false,error:'TOKEN_REQUIRED'},401);try{await verifyGitHubOidc(token);}catch(e){const code=String(e?.message||'OIDC_INVALID');if(code==='JWKS_UNAVAILABLE')return json({ok:false,error:code},503);return json({ok:false,error:'OIDC_INVALID'},403);}const raw=new URL(request.url).searchParams.get('days');const days=raw==null?30:Number(raw);if(!DAYS.has(days))return json({ok:false,error:'INVALID_DAYS'},400);if(!env.DB)return json({ok:false,error:'DB_UNAVAILABLE'},503);try{return json(await collectEvidence(env.DB,days),200,{'cache-control':'no-store'});}catch{return json({ok:false,error:'DB_UNAVAILABLE'},503);}}
